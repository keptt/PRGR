...
def solve_all(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, u1min, u1max, u2min, u2max
    ...
    y1 = 48.1197 + x1*u1_res*(-0.000747192) + x1**2*0.00613734 + x10*u1_res*0.000618299
    y3 = -1.53931 + x1*0.0281138 + x3*u1_res*0.00413477 + x14*u1_res*(-2.98551e-05)
    y4 = -0.884918 + x4*u2_res*0.0615484 + x5*x13*(-0.000378937) + x6*x7*0.000919831
    y5 = -45.0064 + x5*x8*0.00240169 + u1_res*1.12033 + u1_res*x8*(-0.00361076)
    y7 = 24.2162 + x5*x11*0.000320931 + x7*u2_res*0.0791588 + u1_res*(-0.112999)
    y11 = -12.6811 + x2*17.5335 + x7*u1_res*(-0.000768576) + x11*u2_res*0.0899369
...
def main(inputs):
    json_data = solve_all(*inputs)
    # json_data = solve_all(*inputs[1:17], 50, 400, 10, 14, 82, 112, 1, 50000.5, 50, 150, 60, 120, 60, 130, 60, 140, 0.1, 5.5)
